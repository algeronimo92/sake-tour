...
class SakeShopViewModelTest {
    ...
    @Before
    fun setup() {
        val fakeRepo = object : SakeShopRepository {
    ...
    @Test
    fun `loadSakeShops should emit loading and then data`() = runTest {
    ...
    @Test
    fun `loadSakeShops should emit error when use case fails`() = runTest {
        val failingRepo = object : SakeShopRepository {
    ...
    @Test
    fun `findShopByName should return correct shop`() = runTest {
        advanceUntilIdle()
